...
public class TFIDF {
    ...
    public static DocumentData createDocumentDataList(List<String> words, List<String> terms) {
    ...
    private static double getInverseDocumentFrequency(String term, Map<String, DocumentData> documentResults) {
        ...
        return nt == 0 ? 0 : Math.log(documentResults.size() / nt);
    }
    ...
    public static List<String> getWordsOfLine(String line) {
        return Arrays.asList(line.split("(\\.)+|(,)+|( )+|(-)+|(\\?)+|(!)+|(;)+|(:)+|(/d)+|(/n)+"));
    }

    public static List<String> getWordsOfLines(List<String> lines) {
        List<String> words = new ArrayList<>();
        for (String line : lines) {
            words.addAll(getWordsOfLine(line));
        }
